...
function render(){
  ...
  // Goteo: actualización rápida sin reconstruir todo
...
function renderGoteo(){
  ...
  if(res){
    ...
    if(res.modo==="tiempo")
      resumenClinico=`Para administrar ${res.vol} mL a una velocidad de ${res.flujo} mL/h, la infusión tendrá una duración aproximada de ${fStr}. Utilizando un macrogotero de ${res.factor} gotas/mL deberá regularse a ${res.macroGtt} gotas por minuto. Con microgotero deberá regularse a ${res.microGtt} microgotas por minuto.`;
    else
      resumenClinico=`Para administrar ${res.vol} mL en ${fStr}, se necesita un flujo de ${res.flujo} mL/h. Con macrogotero de ${res.factor} gotas/mL: ${res.macroGtt} gotas/min. Con microgotero: ${res.microGtt} microgotas/min.`;
  }
