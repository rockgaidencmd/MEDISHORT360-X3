/* ====================================================
   MEDISHORT360 - CALCULO DE DOSIS EN mL  (app.js)
   Regla de tres para prescripciones medicas IV
   ==================================================== */

const LOGO_B64 = "data:image/jpeg;base64,";

// Factores de conversion a mg
const UNIT_TO_MG = { mg: 1, g: 1000, mcg: 0.001, U: 1, mEq: 1 };
const UNITS = ["mg", "g", "mcg", "U", "mEq"];

// Presets de medicamentos comunes
const PRESETS = [
  { label: "Paracetamol IV",  tipo: "liquido", contenido: 1000, contUnit: "mg", volumen: 100 },
  { label: "Tramadol",        tipo: "liquido", contenido: 100,  contUnit: "mg", volumen: 2   },
  { label: "Metamizol (1g)",  tipo: "liquido", contenido: 1,    contUnit: "g",  volumen: 2   },
  { label: "Gentamicina 80",  tipo: "liquido", contenido: 80,   contUnit: "mg", volumen: 2   },
  { label: "Ranitidina",      tipo: "liquido", contenido: 50,   contUnit: "mg", volumen: 2   },
  { label: "Ceftriaxona 1g",  tipo: "polvo",   contenido: 1,    contUnit: "g",  volumen: ""  },
  { label: "Ampicilina 1g",   tipo: "polvo",   contenido: 1,    contUnit: "g",  volumen: ""  },
  { label: "Amikacina 500",   tipo: "liquido", contenido: 500,  contUnit: "mg", volumen: 2   },
];

// ── Estado global ──────────────────────────────────
let state = {
  drugName:     "",
  dosis:        "",
  dosisUnit:    "mg",
  tipo:         "liquido",  // "liquido" | "polvo"
  // Liquido
  contenido:    "",
  contenidoUnit:"mg",
  volumen:      "",
  // Polvo
  polvoContenido:    "",
  polvoContenidoUnit:"g",
  polvoDisolvente:   "",
  // PWA
  installPrompt: null,
};

function setState(patch) {
  state = { ...state, ...patch };
  render();
}

function el(id) { return document.getElementById(id); }

// ── Lógica de cálculo ──────────────────────────────
function toMg(value, unit) {
  return parseFloat(value || 0) * (UNIT_TO_MG[unit] || 1);
}

function fmt(n) {
  if (n === null || n === undefined || isNaN(n)) return "—";
  if (n >= 100) return n.toFixed(1);
  if (n >= 10)  return n.toFixed(2);
  return n.toFixed(2);
}

function calcular() {
  const dosisMg = toMg(state.dosis, state.dosisUnit);
  if (!dosisMg || dosisMg <= 0) return null;

  let contMg, volMl;

  if (state.tipo === "liquido") {
    contMg = toMg(state.contenido, state.contenidoUnit);
    volMl  = parseFloat(state.volumen || 0);
  } else {
    contMg = toMg(state.polvoContenido, state.polvoContenidoUnit);
    volMl  = parseFloat(state.polvoDisolvente || 0);
  }

  if (!contMg || contMg <= 0 || !volMl || volMl <= 0) return null;

  const mlAdmin = (dosisMg * volMl) / contMg;
  const viales  = dosisMg / contMg;

  return { dosisMg, contMg, volMl, mlAdmin, viales };
}

// ── Partículas ─────────────────────────────────────
function makeParticles() {
  const c = el("particles");
  c.innerHTML = "";
  for (let i = 0; i < 30; i++) {
    const d = document.createElement("div");
    d.className = "particle";
    d.style.cssText = `
      left:${(i*41+9)%100}%;top:${(i*57+13)%100}%;
      width:${(i%3)*0.9+0.4}px;height:${(i%3)*0.9+0.4}px;
      --dur:${((i%4)*1.3+2).toFixed(1)}s;
      --del:${((i%6)*0.55).toFixed(1)}s;
    `;
    c.appendChild(d);
  }
}

// ── Render principal ────────────────────────────────
function render() {
  const app = el("app");
  const res = calcular();

  const bl  = "#1976d2";
  const blt = "#42a5f5";
  const cy  = "#00b4d8";

  // Pasos explicativos
  const pasos = res ? (() => {
    const s = [];
    const dosStr   = `${state.dosis} ${state.dosisUnit}`;
    const drug     = state.drugName || "el medicamento";

    if (state.tipo === "liquido") {
      s.push(`Verifique el vial: contiene ${state.contenido} ${state.contenidoUnit} en ${state.volumen} mL.`);
    } else {
      s.push(`El frasco de polvo contiene ${state.polvoContenido} ${state.polvoContenidoUnit}. Fue diluido en ${state.polvoDisolvente} mL.`);
    }

    s.push(`La prescripcion indica ${dosStr} de ${drug}.`);

    if (res.viales > 1) {
      const vFull = Math.floor(res.viales);
      const resto = res.mlAdmin - vFull * res.volMl;
      s.push(`Necesita ${vFull} vial(es) completo(s) + ${fmt(Math.max(resto, 0))} mL adicionales de otro vial.`);
    }

    s.push(`Administre <strong style="color:${cy}">${fmt(res.mlAdmin)} mL</strong> de la solucion preparada por via IV.`);
    return s;
  })() : null;

  app.innerHTML = `
    <!-- HEADER -->
    <div class="header">
      <div class="badge-row">
        <span class="badge">MEDISHORT360</span>
        <span class="badge">Calculo IV</span>
        <span class="badge">Regla de Tres</span>
      </div>

      <div class="logo-wrap">
        <div class="logo-ring-spin"></div>
        <div class="logo-ring-pulse"></div>
        <img src="${LOGO_B64}" alt="MEDISHORT360" class="logo-img"/>
      </div>

      <div class="app-title">Dosis en mL</div>
      <div class="app-sub">Calcula cuanto administrar segun la prescripcion medica</div>
      <div class="app-brand">📺 Canal MEDISHORT360</div>
    </div>

    <!-- SECCION 1: PRESCRIPCION -->
    <div class="card" style="border-color:${bl}44;box-shadow:0 0 18px ${bl}15">
      <div class="card-label">📋 Prescripcion medica</div>

      <div class="input-wrap" style="margin-bottom:12px">
        <span class="input-label">Nombre del medicamento (opcional)</span>
        <div class="input-row" style="border-color:${bl}44">
          <input type="text" placeholder="Ej: Ceftriaxona, Paracetamol IV..."
            value="${state.drugName}"
            oninput="setState({drugName:this.value})"
            style="font-size:14px;font-family:var(--font);padding:11px 14px"/>
        </div>
      </div>

      <div class="input-wrap">
        <span class="input-label">Dosis prescrita</span>
        <div class="input-row" style="border-color:${bl}44">
          <input type="number" placeholder="Ej: 650" step="any" min="0"
            value="${state.dosis}"
            oninput="setState({dosis:this.value})"/>
          <select class="unit-select"
            onchange="setState({dosisUnit:this.value})">
            ${UNITS.map(u=>`<option value="${u}" ${state.dosisUnit===u?"selected":""}>${u}</option>`).join("")}
          </select>
        </div>
      </div>
    </div>

    <!-- SECCION 2: PRESENTACION -->
    <div class="card" style="border-color:${bl}44;box-shadow:0 0 18px ${bl}15">
      <div class="card-label">💊 Presentacion del medicamento</div>

      <!-- Presets rapidos -->
      <div class="card-label" style="margin-bottom:8px">Acceso rapido:</div>
      <div class="presets-row">
        ${PRESETS.map(p=>`
          <button class="preset-btn" onclick="applyPreset(${JSON.stringify(p).replace(/"/g,"'")})">
            ${p.label}
          </button>
        `).join("")}
      </div>

      <div class="divider">o ingresa manualmente</div>
      <div style="height:14px"></div>

      <!-- Toggle tipo -->
      <div class="tipo-row" style="margin-bottom:16px">
        <button class="tipo-btn ${state.tipo==="liquido"?"active":""}"
          onclick="setState({tipo:'liquido'})"
          style="${state.tipo==="liquido" ? `background:${bl}22;border-color:${blt};color:#fff` : ""}">
          <span class="tipo-icon">💉</span>
          Liquido<br><small style="font-size:10px;opacity:.7">Ya viene diluido</small>
        </button>
        <button class="tipo-btn ${state.tipo==="polvo"?"active":""}"
          onclick="setState({tipo:'polvo'})"
          style="${state.tipo==="polvo" ? `background:${bl}22;border-color:${blt};color:#fff` : ""}">
          <span class="tipo-icon">🧪</span>
          Polvo<br><small style="font-size:10px;opacity:.7">Reconstituido</small>
        </button>
      </div>

      ${state.tipo === "liquido" ? `
        <!-- LIQUIDO -->
        <div class="grid-2" style="align-items:end">
          <div class="input-wrap">
            <span class="input-label">Contenido total del vial</span>
            <div class="input-row" style="border-color:${bl}44">
              <input type="number" placeholder="Ej: 1000" step="any" min="0"
                value="${state.contenido}"
                oninput="setState({contenido:this.value})"/>
              <select class="unit-select"
                onchange="setState({contenidoUnit:this.value})">
                ${UNITS.map(u=>`<option value="${u}" ${state.contenidoUnit===u?"selected":""}>${u}</option>`).join("")}
              </select>
            </div>
          </div>
          <div class="input-wrap">
            <span class="input-label">Volumen del vial</span>
            <div class="input-row" style="border-color:${bl}44">
              <input type="number" placeholder="Ej: 100" step="any" min="0"
                value="${state.volumen}"
                oninput="setState({volumen:this.value})"/>
              <span class="input-unit" style="color:${blt}">mL</span>
            </div>
          </div>
        </div>
        <div class="helper-text">
          Ej: Paracetamol IV → <strong>1000 mg</strong> en <strong>100 mL</strong><br>
          Ej: Tramadol → <strong>100 mg</strong> en <strong>2 mL</strong>
        </div>
      ` : `
        <!-- POLVO RECONSTITUIDO -->
        <div class="input-wrap" style="margin-bottom:12px">
          <span class="input-label">Contenido del frasco (polvo)</span>
          <div class="input-row" style="border-color:${bl}44">
            <input type="number" placeholder="Ej: 1" step="any" min="0"
              value="${state.polvoContenido}"
              oninput="setState({polvoContenido:this.value})"/>
            <select class="unit-select"
              onchange="setState({polvoContenidoUnit:this.value})">
              ${UNITS.map(u=>`<option value="${u}" ${state.polvoContenidoUnit===u?"selected":""}>${u}</option>`).join("")}
            </select>
          </div>
        </div>
        <div class="input-wrap">
          <span class="input-label">Fue disuelto / diluido en</span>
          <div class="input-row" style="border-color:${bl}44">
            <input type="number" placeholder="Ej: 10" step="any" min="0"
              value="${state.polvoDisolvente}"
              oninput="setState({polvoDisolvente:this.value})"/>
            <span class="input-unit" style="color:${blt}">mL</span>
          </div>
        </div>
        <div class="helper-text">
          Ej: Ceftriaxona 1g → frasco de <strong>1 g</strong> disuelto en <strong>10 mL</strong><br>
          Ej: Ampicilina 1g → frasco de <strong>1 g</strong> disuelto en <strong>5 mL</strong>
        </div>
      `}
    </div>

    <!-- ERROR: campos vacios -->
    ${(!res && (state.dosis || state.contenido || state.polvoContenido)) ? `
      <div class="error-box">
        ⚠️ Completa todos los campos para calcular la dosis.
      </div>
    ` : ""}

    <!-- ADVERTENCIA: supera un vial -->
    ${res && res.viales > 1 ? `
      <div class="warn-box">
        ⚠️ La dosis prescrita supera <strong>un vial</strong>.
        Necesitas <strong>${res.viales.toFixed(2)} viales</strong> en total.
        Verifica la orden medica antes de continuar.
      </div>
    ` : ""}

    <!-- RESULTADO -->
    ${res ? `
      <div class="result-card" style="border-color:${blt}55;box-shadow:0 0 30px ${bl}22">
        <div class="result-header">
          <div class="card-label">Regla de Tres</div>
          <div class="r3-display">
            <div class="r3-title">Si el vial tiene...</div>
            <table class="r3-table">
              <tr>
                <td class="r3-lbl">Si</td>
                <td class="r3-amount">${fmt(res.contMg)} mg</td>
                <td class="r3-arrow">─────</td>
                <td class="r3-vol">${fmt(res.volMl)} mL</td>
              </tr>
              <tr><td colspan="4"><hr class="r3-sep"/></td></tr>
              <tr class="highlight">
                <td class="r3-lbl">X</td>
                <td class="r3-amount">${fmt(res.dosisMg)} mg</td>
                <td class="r3-arrow">─────</td>
                <td class="r3-vol">? mL</td>
              </tr>
            </table>
            <div class="r3-formula">
              X = (${fmt(res.dosisMg)} &times; ${fmt(res.volMl)}) &divide; ${fmt(res.contMg)}
              = <strong>${fmt(res.mlAdmin)} mL</strong>
            </div>
          </div>
        </div>

        <div class="result-body">
          <div class="result-main">
            <div style="font-size:11px;text-transform:uppercase;letter-spacing:1px;color:var(--muted);margin-bottom:8px">
              Administrar
            </div>
            <div class="result-number" style="color:${cy};text-shadow:0 0 20px ${cy}55">
              ${fmt(res.mlAdmin)}<span class="result-unit" style="color:${blt}">mL</span>
            </div>
            <div class="result-desc">
              de ${state.drugName || "la solucion"} —
              ${state.dosis} ${state.dosisUnit} prescritos
            </div>
          </div>

          <!-- Barra proporcional -->
          <div style="font-size:10px;color:var(--muted);margin-bottom:4px;margin-top:12px">
            ${fmt(res.mlAdmin)} mL de ${fmt(res.volMl)} mL del vial
            (${Math.min(res.viales*100, 100).toFixed(0)}%${res.viales>1?" — supera 1 vial":""})
          </div>
          <div class="dose-bar-wrap">
            <div class="dose-bar" style="
              width:${Math.min(res.viales*100, 100).toFixed(1)}%;
              background:linear-gradient(90deg,${bl},${cy});
            "></div>
          </div>

          ${res.viales > 1 ? `
            <div class="vial-warn">
              <span class="vial-warn-icon">🏥</span>
              <div class="vial-warn-text">
                Se necesitan <strong>${res.viales.toFixed(2)} viales</strong>.
                Prepara ${Math.ceil(res.viales)} frascos y usa solo los mL calculados.
              </div>
            </div>
          ` : ""}
        </div>
      </div>

      <!-- PASOS -->
      <div class="steps-card"
        style="border-color:${blt};background:linear-gradient(135deg,#091928,#040c1f);
               box-shadow:0 0 28px ${bl}33">
        <div class="steps-title" style="color:${blt}">
          <span>🩺</span> Procedimiento paso a paso
        </div>
        <div class="steps-list">
          ${pasos.map((p,i)=>`
            <div class="step-row">
              <div class="step-num"
                style="background:linear-gradient(135deg,${bl},${cy});
                       box-shadow:0 0 8px ${bl}66">${i+1}</div>
              <p class="step-text">${p}</p>
            </div>
          `).join("")}
        </div>
      </div>
    ` : ""}

    <!-- FOOTER -->
    <div class="footer">
      <p>Solo para referencia clinica &middot; Verificar con prescripcion original</p>
      <strong>🩺 MEDISHORT360 &middot; Canal Medico Educativo</strong>
    </div>
  `;
}

// ── Aplicar preset ─────────────────────────────────
window.applyPreset = function(p) {
  setState({
    tipo:              p.tipo,
    contenido:         p.tipo === "liquido" ? String(p.contenido) : "",
    contenidoUnit:     p.tipo === "liquido" ? p.contUnit : "mg",
    volumen:           p.tipo === "liquido" ? String(p.volumen) : "",
    polvoContenido:    p.tipo === "polvo"   ? String(p.contenido) : "",
    polvoContenidoUnit:p.tipo === "polvo"   ? p.contUnit : "g",
    polvoDisolvente:   "",
    drugName:          p.label,
  });
};

// ── PWA install ─────────────────────────────────────
window.addEventListener("beforeinstallprompt", (e) => {
  e.preventDefault();
  state.installPrompt = e;
  el("install-btn").classList.add("visible");
});
window.addEventListener("appinstalled", () => {
  el("install-btn").classList.remove("visible");
  state.installPrompt = null;
});
window.handleInstall = function() {
  if (!state.installPrompt) return;
  state.installPrompt.prompt();
  state.installPrompt = null;
  el("install-btn").classList.remove("visible");
};

// ── Service Worker ───────────────────────────────────
if ("serviceWorker" in navigator) {
  window.addEventListener("load", () => {
    navigator.serviceWorker.register("./sw.js")
      .then(() => console.log("SW Dosis360 registrado"))
      .catch(e => console.log("SW error:", e));
  });
}

// ── Init ─────────────────────────────────────────────
document.addEventListener("DOMContentLoaded", () => {
  makeParticles();
  render();
});
